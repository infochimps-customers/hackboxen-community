...
If a requirements declaration exists for a hackbox and a "sanity check" is run for that hackbox, the @requires@ and @provides trees are compared as follows: A @provides@ key must also exist in @Settings@ and have as its value a hash tree which "covers" every key in the @requires@ tree.  If a key is missing from @provides@, then an error is raised (indicating the missing keys) and the hackbox will fail to run.
